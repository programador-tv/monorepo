...
function loadMentoresParaTag(data) {
  ...
  if (data.length == 0) {
    prepared =
      "<div style='justify-content: center; display: flex; padding: 20px; color: grey;'>Sem mentorias no momento <div>";
    ...
    for (const profileWithTimeSelections of data) {
      ...
      groupOneToOne = [
        ...
        ...profileWithTimeSelections.timeSelections
          .filter((x) => x.variacao == 1 || x.variacao == 0)
          .map((e) => {
    ...
    for (let item in groupOneToOne)
      preparedOneToOne += JoinTimeForMentorias(
        groupOneToOne[item].ts,
        groupOneToOne[item].perfil,
        "groupOneToOne",
        item
      );
    for (let item in groupCursos)
      preparedCursos += JoinTimeForMentorias(
        groupCursos[item].ts,
        groupCursos[item].perfil,
        "groupCursos",
        item
      );
    ...
    if (groupOneToOne.length > 0) {
      countOneToOneElement
        .html(groupOneToOne.length)
        .addClass("count-tab-events");
    }

    $("#CursosMatch").html(preparedCursos);
    if (groupCursos.length > 0) {
      countCursosElement.html(groupCursos.length).addClass("count-tab-events");
    }
...
function loadPedidosAjudaParaTag(data) {
  ...
  if (data.length == 0) {
    ...
    for (let item in groupHelp)
      pedidosDeAjuda += JoinTimeForRequestedHelp(
        groupHelp[item].ts,
        groupHelp[item].perfil,
        "groupHelp",
        item
      );
    ...
    if (groupHelp.length > 0) {
      countHelpElement.html(groupHelp.length).addClass("count-tab-events");
    }
...
function activePaginationFor(group, length) {
  ...
  if (length <= size) {
    ...
    left.style.filter = "grayscale(0.8)";
    return;
  }
