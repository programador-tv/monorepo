function enviarTagsParaAPI(tags) {
  ...
  fetch("?handler=ChatGPT&entrada=" + tags)
    .then(function (response) {
      document
        .querySelector("#Titulo-Sched")
        .classList.remove("skeleton-loading");
      document
        .querySelector("#TituloDescricao-Sched")
        .classList.remove("skeleton-loading");
...
function verificarSelecaoLiveSchedule() {
  let selectedTags = [];
  for (let i = 0; i < selectElementScheduleLive.options.length; i++) {
  ...
  if (selectedTags.length > 0) {
    ...
    document
      .getElementById("btnPrepararChatGPT-Sched")
...
document
  ...
  .addEventListener("click", function (event) {
    ...
    const selectedTags = [];
    for (let i = 0; i < selectElementScheduleLive.options.length; i++) {
      const option = selectElementScheduleLive.options[i];
      if (option.selected) {
...
const selectElementScheduleLive = document.querySelector(
  "#TagsForScheduleLive"
);
