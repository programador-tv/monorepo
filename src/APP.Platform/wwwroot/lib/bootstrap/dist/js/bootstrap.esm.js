...
const EVENT_defaultDOWN = `defaultdown${EVENT_KEY$a}`;
const EVENT_defaultUP = `defaultup${EVENT_KEY$a}`;
...
const CLASS_NAME_default_EVENT = "default-event";
...
const default_TYPE_TOUCH = "touch";
const default_TYPE_PEN = "pen";
/**
 * ------------------------------------------------------------------------
 * Class Definition
 * ------------------------------------------------------------------------
...
class Carousel extends BaseComponent {
  constructor(element, config) {
    ...
    this._defaultEvent = Boolean(window.defaultEvent);
  ...
  _addTouchEventListeners() {
    const start = (event) => {
      if (
        this._defaultEvent &&
        (event.defaultType === default_TYPE_PEN ||
          event.defaultType === default_TYPE_TOUCH)
      ) {
        this.touchStartX = event.clientX;
      } else if (!this._defaultEvent) {
    ...
    if (this._defaultEvent) {
      EventHandler.on(this._element, EVENT_defaultDOWN, (event) =>
        start(event)
      );
      EventHandler.on(this._element, EVENT_defaultUP, (event) => end(event));

      this._element.classList.add(CLASS_NAME_default_EVENT);
    } else {
